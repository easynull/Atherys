...
public abstract class Research<O extends Research<O>> {
    ...
    private void add(O research) {
        if(research instanceof Chapter c) LRResearches.chapters.add(c);
        if(research instanceof Entry e) LRResearches.entries.add(e);
        LRResearches.researchById.put(getID(), research);
    }
}
