...
public final class Entry extends Research<Entry> {
    ...
    public void onDraw(ResourceLocation bg, GuiGraphics gg, int pX, int pY, int mouseX, int mouseY) {
        if(chapter == currentChap){
            ...
            RenderUtils.drawText(LetherianLang.translate(Component.literal(getName()), isUnlocked()), gg, pX + 17, pY + 4, 0xFF4DBE);
            tr.stop();
            if (isUnlocked()) {
                ...
                RenderUtils.drawTexture(bg, gg, pX + 3, pY + 2, 280, 32, 12, 12, 512, 512);
            }
        }
    }
