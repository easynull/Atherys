package com.easynull.lethifer.core.commands;
...
public class ResearchCommands {
    ...
    private static CompletableFuture<Suggestions> suggestResearch(SuggestionsBuilder builder) {
        LRResearches.researchById.values().forEach(r -> builder.suggest(r.getID()));
    ...
    private static int give(CommandSourceStack source, String researchId) {
        ServerPlayer player = source.getPlayer();
        if (player == null) return 0;
        ItemStack stack = new ItemStack(LRItemsBlocks.ancientPage.asItem());
        stack.set(LRComponents.research, researchId);
