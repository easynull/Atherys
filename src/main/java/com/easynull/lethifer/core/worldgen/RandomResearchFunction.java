...
public class RandomResearchFunction extends LootItemConditionalFunction {
    ...
    protected ItemStack run(ItemStack stack, LootContext context) {
        List<Entry> entries = LRResearches.entries;
        if(!entries.isEmpty()){
            RandomSource rand = context.getRandom();
            Entry entry = entries.get(rand.nextInt(entries.size()));
            if (stack.is(LRItemsBlocks.ancientPage)) {
                stack = new ItemStack(LRItemsBlocks.ancientPage.asItem());
            }
            if(!entry.primal) {
                stack.set(LRComponents.research, entry.getID());
            }
        }
        return stack;
    }
