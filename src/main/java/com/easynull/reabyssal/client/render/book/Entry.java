...
import static com.easynull.reabyssal.client.render.book.BookScreen.curChap;
    ...
    public void onRender(ResourceLocation bg, GuiGraphics gg, int pX, int pY, int mouseX, int mouseY, boolean left) {
        if (chapter == curChap) {
            RenderUtils.rePaint(isUnlocked() ? 0xFFFFFFFF : 0xFFA9A9A9, ()-> {
                ...
                RenderUtils.drawText(ReAbyssal.translation("entry", name), gg, pX + 28, pY + 5, 0xFF7F2D1B, Minecraft.getInstance().font);
                tr.stop();
            });
            if (isUnlocked()) {
